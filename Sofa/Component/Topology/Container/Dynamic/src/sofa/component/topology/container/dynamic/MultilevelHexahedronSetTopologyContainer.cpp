...
void MultilevelHexahedronSetTopologyContainer::clear()
{
    ...
    for(auto i : _coarseComponents.getValue())
        delete i;
        ...
        type::vector<Index> &neighbors)
{
    ...
    for(int vertexId=0; vertexId<8; ++vertexId)
    {
        ...
        for(unsigned int vneighbor : vneighbors)
            uniqueNeighbors.insert(vneighbor);
    }
        ...
        type::vector<Index> &neighbors)
{
    ...
    for(unsigned int i : vertexShell)
    {
        if(i != hexa)
            neighbors.push_back(i);
    }
