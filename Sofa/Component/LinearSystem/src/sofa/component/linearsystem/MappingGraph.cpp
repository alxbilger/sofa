...
void MappingGraph::computeJacobianMappingMatrices(
    core::ConstraintParams* cparams,
    BaseMechanicalState* mappingOutput,
    const sofa::type::vector<unsigned>& dofsIds,
    core::MatrixDerivId& id) const
{
    simulation::mechanicalvisitor::MechanicalResetConstraintVisitor(cparams).execute(m_rootNode);
    mappingOutput->buildIdentityBlocksInJacobian(dofsIds, id);
    simulation::mechanicalvisitor::MechanicalAccumulateJacobian(cparams, id).execute(m_rootNode);
}
