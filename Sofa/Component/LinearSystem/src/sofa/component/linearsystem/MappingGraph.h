...
public:
    ...
    /**
     * @brief Compute the jacobian matrices of mapppings based on a provided mapping output
     * @details The jacobian matrices of all mappings which have a direct connection to the provided
     * mechanical state are computed. The matrices can be filtered to only a subset of the DOFs in
     * the provided mechanical state. The matrices are stored in the provided id in each connected
     * mechanical state.
     */
    void computeJacobianMappingMatrices(
        core::ConstraintParams* cparams,
        BaseMechanicalState* mappingOutput,
        const sofa::type::vector<unsigned int>& dofsIds,
        core::MatrixDerivId& id)
    const;
