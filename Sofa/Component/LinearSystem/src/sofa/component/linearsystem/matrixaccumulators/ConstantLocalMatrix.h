...
class ConstantLocalMatrix : public virtual AssemblingMatrixAccumulator<TComponentType>
{
public:
    SOFA_CLASS(ConstantLocalMatrix, AssemblingMatrixAccumulator<TComponentType>);
...
void ConstantLocalMatrix<ComponentType>::add(const core::matrixaccumulator::no_check_policy&, sofa::SignedIndex row, sofa::SignedIndex col, float value)
{
...
template <class ComponentType>
void ConstantLocalMatrix<ComponentType>::add(const core::matrixaccumulator::no_check_policy&, sofa::SignedIndex row, sofa::SignedIndex col, double value)
{
    static_cast<ConstantSparsityPatternSystem::Matrix*>(this->m_globalMatrix)->colsValue[insertionOrderList[currentId++]]
        += this->m_cachedFactor * value;
}
...
template <class ComponentType>
void ConstantLocalMatrix<ComponentType>::add(const core::matrixaccumulator::no_check_policy&, sofa::SignedIndex row, sofa::SignedIndex col,
    ...
    const sofa::type::Mat<3, 3, double>& value)
{
    for (sofa::SignedIndex i = 0; i < 3; ++i)
    {
        for (sofa::SignedIndex j = 0; j < 3; ++j)
        {
            this->Inherit1::add(core::matrixaccumulator::no_check, row + i, col + j, value(i, j));
        }
    }
}
