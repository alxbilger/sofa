...
void MatrixLinearSolver<Matrix, Vector, NoThreadManager>::doCheckLinearSystem()
{
    if (!l_linearSystem)
    {
        ...
        if (listLinearSystems.empty())
        {
            ...
            {
                msg_warning() << "A linear system is required, but has not been found. Add a linear system to your scene to "
...
void MatrixLinearSolver<Matrix,Vector>::createDefaultLinearSystem()
{
    ...
    {
        if (!this->getContext()->addObject(system))
        {
            msg_warning() << TLinearSystemType::GetClass()->className << " failed to be added in the current context";
        }
