...
void VisualModelImpl::updateVisual()
{
    if (modified && !getVertices().empty())
    {
        ...
        {
            SCOPED_TIMER("VisualModelImpl::computePositions");
            computePositions();
        }
        {
            SCOPED_TIMER("VisualModelImpl::computeNormals");
            computeNormals();
        }
        if (m_updateTangents.getValue())
        {
            SCOPED_TIMER("VisualModelImpl::computeTangents");
            computeTangents();
        }
        if (m_vtexcoords.getValue().size() == 0)
        {
            SCOPED_TIMER("VisualModelImpl::computeUVSphereProjection");
            computeUVSphereProjection();
        }
        {
            SCOPED_TIMER("VisualModelImpl::updateBuffers");
            updateBuffers();
        }
