...
template <class DataTypes>
void TriangleFEMForceField<DataTypes>::buildStiffnessMatrix(sofa::core::behavior::StiffnessMatrixAccumulator* matrices)
{
            ...
            for(unsigned j = 0; j < S; j++)
            {
                ...
                for (unsigned n2=0; n2<nodeIndex.size(); n2++)
                {
                    for (unsigned k=0; k<S; k++)
                    {
                        ...
                        matrices->add( ROW,COLUMN, - RJKJtRt[row][column] );
                    }
                }
            }
        }
