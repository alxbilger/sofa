...
template <class DataTypes>
void TetrahedronFEMForceField<DataTypes>::buildStiffnessMatrix(sofa::core::behavior::StiffnessMatrixAccumulator* matrices)
{
    int IT = 0;
    StiffnessMatrix JKJt,tmp;
    ...
    for(auto it = _indexedElements->begin() ; it != _indexedElements->end() ; ++it,++IT)
    {
        ...
        for (sofa::Index n1=0; n1 < N; n1++)
        {
            for (sofa::Index n2=0; n2 < N; n2++)
            {
                matrices->add( (*it)[n1] * S, (*it)[n2] * S, tmpBlock[n1][n2]);
            }
        }
    }
}
