...
const std::string SceneCheckMapping::getDesc() { return "Check if the mappings and states inside a Node are consistent."; }
    ...
    const auto showNodes = [](std::stringstream& ss, sofa::type::vector<sofa::simulation::Node*>& nodes)
    {
        for (auto node : nodes)
        {
            ...
            if (node->mechanicalMapping != nullptr)
            {
                ...
                if (!list.empty())
                {
                    ss << " (it is advised to have [" << list << "] state(s) and " << node->mechanicalMapping->getPathName() << " in the same Node";
                }
            }
            ss << msgendl;
        }
